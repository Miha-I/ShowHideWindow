...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_SHIFT:
		{
			if (enab_dis)
			{
				h = FindWindow(TEXT("CalcFrame"), TEXT("Калькулятор"));
				if (h)
				{
				...
				{
					MessageBox(hWnd, TEXT("Откройте калькулятор, пожалуйста"), TEXT(""), 0);
				}
			}
			else
			{
    ...
    case WM_PAINT:
        {
			...
			if (enab_dis)
				TextOut(hdc, 0, 20, L"Запустите калькулятор и нажмите Shift", lstrlen(L"Запустите калькулятор и нажмите Shift"));
			else
				TextOut(hdc, 0, 20, L"Для отключения нажмите Shift ещё раз", lstrlen(L"Для отключения нажмите Shift ещё раз"));
